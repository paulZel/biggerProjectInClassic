...
var burger = function burger(menuSelector, burgerSelector) {
  var menuElem = document.querySelector(menuSelector),
      burgerElem = document.querySelector(burgerSelector); //Вручную скроем меню чтобы оно случайно при инициализации не оказалось в пользователя на экране

  menuElem.style.display = 'none'; //следующие отслежувание действий на бургере

  burgerElem.addEventListener('click', function () {
    if (menuElem.style.display == "none" && window.screen.availWidth < 993) {
      //без технических характреристик например полоса прокрутки, только контент
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var sum = 0; //функция котораябудет срабатывать при выборе елемента пользователем
  ...
  var calcFunc = function calcFunc() {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      ...
      //два первых селекта заполнено но мы должны также проверить промокод
...
var filter = function filter() {
  //будет слишком много классов в верстке чтобы передавать их аргументами. Пропишем вручную.
  var menu = document.querySelector('.portfolio-menu'),
      ...
      no = document.querySelector('.portfolio-no'); //нужна сделать фильтрацию елементов, показывать только нужные
  ...
  var typeFilter = function typeFilter(markType) {
    ...
    no.classList.remove('animated', 'fadeIn'); //таким образом будет показывать интересные пользователю елементы
  ...
  menu.addEventListener('click', function (e) {
    var target = e.target; //елемент на катором будет происходить событие

    if (target && target.tagName == "LI") {
      //обьязательно верхний регистр
      items.forEach(function (btn) {
        return btn.classList.remove('active');
      }); //удаляем класс акивности со всех елементов в меню
...
var forms = function forms() {
  //собирет информацию и отправлеет на сервер
  ...
  upload.forEach(function (item) {
    item.addEventListener('input', function () {
      ...
      var dots; //либо будет содержать ... либо ничего
      //повторяющийся код закинем в переменую.

      var arr = item.files[0].name.split('.'); //условие на длину имя файла, сколько символов показывать

      arr[0].length > 5 ? dots = "..." : dots = '.'; //теперь формируем само имя
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      //отправляя форму она перезагружается это нужно отменить для adjax 
      e.preventDefault();
      ...
      }, 400); //теперь кроме того что форма станет прозрачной, она еще и исчезнет физически со страницы на время
      //теперь статус оповещения

      var statusImg = document.createElement('img'); //будем создавать тег img
      ...
      statusMessage.appendChild(statusImg); //наш спинер готов и он будет отражатся после того как мы отправляем форму
      ...
      statusMessage.appendChild(textMessage); //собираем все даные с формы

      var formData = new FormData(item);
      var api; //эта конструкция может иметь два выхода, если блок существует в родителя то получим его, если нет получим false по факту null можем это использовать для логического утверждения

      item.closest('.popup-design') || item.classList.contains('calc_form') ? api = path.designer : api = path.question; //если здесь будет больше адрессов можно легко использовать switch case (было в прошлом проекте)

      console.log(api); //postData('assets/server.php', formData).then(res => {
      ...
      .catch(function () {
        ...
        clearInputs();
        setTimeout(function () {
          statusMessage.remove(); //так как стоит display none поставим block

          item.style.display = 'block'; //форма не появится благодаря анимации opecity 0
...
var mask = function mask(selector) {
  ...
  function createMask(event) {
    var matrix = '+38 (___) ___ __ __',
        //матрицу можно вбить вручную, подятягивать с БД или json файл зависимо от страны
    i = 0,
        //удаления всех не цифр внутри матрицы
    def = matrix.replace(/\D/g, ''),
        //статичная на основе матрицы
    ...
    if (event.type === 'blur') {
      ...
      //если не blur, скорее всего тогда focus
...
var modals = function modals() {
  var btnPressed = false; //для вычисления была ли нажата хотя бы одна кнопка
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    var trigger = document.querySelectorAll(triggerSelector),
        ...
        //все модальные окна со станицы
    scroll = calcScroll();
    trigger.forEach(function (item) {
      item.addEventListener('click', function (e) {
        if (e.target) {
          //проверка на обьект ивента
        ...
        windows.forEach(function (item) {
          item.style.display = 'none'; //когда будем скрывать ненужные модальные окна будем использовать дополнительные классы
        ...
        document.body.style.overflow = "hidden"; //document.body.classList.add('modal-open'); //можем использувать анимации и бутстрап css

        document.body.style.marginRight = "".concat(scroll, "px");
      });
    });
    close.addEventListener('click', function () {
      //обект события нам здесь не нужен
      windows.forEach(function (item) {
        item.style.display = 'none';
      });
  ...
  function showModalByTime(selector, time) {
    setTimeout(function () {
      var display; //логически false / undefind
  ...
  function calcScroll() {
    ...
    div.style.visibility = 'hidden'; //этот блок готов и мы можем поместить его на страницу

    document.body.appendChild(div);
    var scrollWidth = div.offsetWidth - div.clientWidth; //от полной ширины отнимаем пединги и главный контент (не включается прокрутка)
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      var scrollHeight = Math.max(document.documentElement.scrollHeight, document.body.scrollHeight); //вернет максимальное значение из аргументов каторые мы передадим
...
var pictureSize = function pictureSize(imgSelector) {
  var blocks = document.querySelectorAll(imgSelector); //мы должны показывать изображения и скрывать их.

  function showImg(block) {
    //мы можем получить картинку в этом блоке
    var img = block.querySelector('img'); //нужно изменить путь src можем напрямую обратится через точку
  ...
  function hideImg(block) {
    //мы можем получить картинку в этом блоке
    var img = block.querySelector('img'); //можно использовать регулярное выражение чтобы вырезать -1, но мы уже используем метод slice
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1,
      //будет отображать текущей слайд, что показывается пользователю.
  paused = false; //переменая должна знать нужно ли в текущий момент остановить переключение слайдов

  var items = document.querySelectorAll(slides); //функция перемещения slideIndex

  function showSlides(n) {
    //можем идти вперед и назад
    if (n > items.length) {
      //чтобы не выходило больше слайдов чем есть
      slideIndex = 1;
    }

    if (n < 1) {
      //чтобы не выходило меньше 0
  ...
  function changeSlide(n) {
    //будем вызывать когда будем кликать на определенные елементы либо вперед либо назад
  ...
  try {
    var prevBtn = document.querySelector(prev),
        nextBtn = document.querySelector(next); //а если код все таки есть прописываем
    ...
    prevBtn.addEventListener('click', function () {
      ...
      items[slideIndex - 1].classList.add('slideInRight'); //нужно это сделать чтобы убирать класс каторый мы добавляем в следующем
    });
      ...
      switch (_context.prev = _context.next) {
        case 0:
          ...
          return regeneratorRuntime.awrap(fetch(url, {
            //булет записыв тут promise каторый вернет серевер Нужно дождатся выполенения запроса
            method: "POST",
            body: data
          }));
